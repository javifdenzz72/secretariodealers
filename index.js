...
const ROLE_ID = "1266928759039066204"; // Reemplaza con el ID del rol específico
...
const INITIAL_COOLDOWN = 7200; // 2 horas en segundos
  ...
  Object.entries(MANAGERS).forEach(([manager, data]) => {
    ...
    Object.entries(data.tasks).forEach(([taskType, taskName]) => {
      if (
        ...
        if (
          completedMessages[manager] &&
          completedMessages[manager][taskType]
        ) {
...
client.login(
  "MTM4ODE3NjI2ODE0MjMxMzU3NQ.GNKDMH.7SF9EBRiqCCmYi8dATkviflTw7JAsGxHyItZ44",
);
